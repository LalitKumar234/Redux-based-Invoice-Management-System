...
import { addInvoice, allInvoices, editInvoice } from '../features/invoiceSlice'
...
class InvoiceForm extends React.Component {
  constructor(props) {
    ...
    console.log('All Invoices:', invoices.payload.invoices);

    const id = window.location.href.split('/')[4];
    const initialInvoice = invoices.payload.invoices.find(invoice => invoice.id === id);

    this.state = {
      currentRoute: window.location.href.split('/')[3],
      isOpen: false,
      currency: initialInvoice?.currency || '$',
      currentDate: initialInvoice?.currentDate || '',
      invoiceNumber: initialInvoice?.invoiceNumber || 1,
      dateOfIssue: initialInvoice?.dateOfIssue || '',
      billTo: initialInvoice?.billTo || '',
      billToEmail: initialInvoice?.billToEmail || '',
      billToAddress: initialInvoice?.billToAddress || '',
      billFrom: initialInvoice?.billFrom || '',
      billFromEmail: initialInvoice?.billFromEmail || '',
      billFromAddress: initialInvoice?.billFromAddress || '',
      notes: initialInvoice?.notes || '',
      total: initialInvoice?.total || '0.00',
      subTotal: initialInvoice?.subTotal || '0.00',
      taxRate: initialInvoice?.taxRate || '',
      taxAmmount: initialInvoice?.taxAmmount || '0.00',
      discountRate: initialInvoice?.discountRate || '',
      discountAmmount: initialInvoice?.discountAmmount || '0.00',
      id: initialInvoice?.id || nanoid()
    };
    this.state.items = initialInvoice?.items || [{
      id: 0,
      name: '',
      description: '',
      price: '1.00',
  ...
  openModal = (event) => {
    ...
    const { currentRoute, isOpen, ...invoiceData } = this.state;

    this.state.currentRoute === 'edit-invoice' ?
    this.props.editInvoice({ id: this.state.id || '', updatedInvoice: invoiceData }):
      this.props.addInvoice({...invoiceData, id: nanoid()}) 
  };
        ...
        <Col md={4} lg={3}>
          ...
          <div className="sticky-top pt-md-3 pt-xl-4">
            <div className='d-flex flex-row align-items-center gap-3'>
              {
                this.state.currentRoute === "edit-invoice" ? 
                <Button variant="primary" type="submit" className="d-block w-100">Update and Save</Button> :
                <Button variant="primary" type="submit" className="d-block w-100">Review and Save</Button>
              }
            </div>
